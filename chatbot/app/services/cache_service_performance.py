...
class PerformanceCacheService:
    ...
    async def get_responses(self, conversation_id: str) -> List[Dict[str, Any]]:
        """
        ...
        if conversation_id in self.loaded_sessions:
            memory_responses = await self.memory_cache.get(conversation_id)
            if memory_responses:
                logger.debug(f"⚡ Fast path: Retrieved {len(memory_responses)} responses from memory for {conversation_id}")
                return memory_responses
            else:
                # Session was loaded but memory is empty (cleared or new)
                logger.debug(f"⚡ Fast path: Empty memory cache for loaded session {conversation_id}")
                return []
        
        # 🔄 WARM-UP PATH: Load from PostgreSQL and cache in memory
        if self.postgres_backend:
            try:
                postgres_responses = await self.postgres_backend.get(conversation_id)
                if postgres_responses:
                    logger.debug(f"🔄 Warm-up: Loading {len(postgres_responses)} responses from PostgreSQL for {conversation_id}")
                    
                    # Cache all responses in memory for future speed
                    for response in postgres_responses:
                        await self.memory_cache.add(conversation_id, response)
                    
                    # Mark as loaded to use fast path next time
                    self.loaded_sessions.add(conversation_id)
                    
                    logger.debug(f"🔄 Warm-up complete: {conversation_id} now cached in memory")
                    return postgres_responses
                else:
                    logger.debug(f"🔄 Warm-up: No data in PostgreSQL for {conversation_id}")
            except Exception as e:
                logger.error(f"PostgreSQL warm-up error for {conversation_id}: {e}")
        
        # 🆕 NEW CONVERSATION: Mark as loaded and start with empty cache
        self.loaded_sessions.add(conversation_id)
        logger.debug(f"🆕 New conversation: {conversation_id} marked as loaded")
        return []
    ...
    async def warm_up_session(self, conversation_id: str) -> bool:
        """
        ...
        """
        if conversation_id in self.loaded_sessions:
            logger.debug(f"🔥 Session {conversation_id} already warmed up")
            return True
        ...
        if self.postgres_backend:
            try:
                ...
                if postgres_responses:
                    ...
                    self.loaded_sessions.add(conversation_id)
                    logger.debug(f"🔥 Warmed up session {conversation_id} with {len(postgres_responses)} responses")
                    return True
            except Exception as e:
                logger.error(f"Warm-up error for {conversation_id}: {e}")
