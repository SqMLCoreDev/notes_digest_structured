...
from medical_notes.routes.validation_routes import router as validation_router
...
async def process_note_with_tracking(job_id: str, note_id: str):
    ...
    try:
        ...
        # Stage 8: Clinical Validation (NEW)
        current_stage = "clinical_validation"
        validation_result = None
        
        try:
            add_log(job_id, "clinical_validation", "in_progress",
                    "Running clinical validation on generated output")
            
            from medical_notes.validation.validator import validate_note
            
            # Use original rawdata (without historical context) as source
            source_note_for_validation = rawdata
            
            # Use processed_text as generated output (or soap_text as fallback)
            generated_output_for_validation = processed_text if processed_text else soap_text
            
            if generated_output_for_validation:
                validation_result = validate_note(
                    source_note=source_note_for_validation,
                    generated_output=generated_output_for_validation,
                    note_type=note_type,
                    note_id=note_id,
                    store_to_es=True
                )
                
                validation_decision = validation_result.get("decision", "UNKNOWN")
                validation_score = validation_result.get("score", 0.0)
                
                add_log(job_id, "clinical_validation", "completed",
                        f"Validation completed: {validation_decision} (score: {validation_score:.3f})")
                
                # Log validation scores
                scores = validation_result.get("scores", {})
                add_log(job_id, "clinical_validation", "info",
                        f"Validation scores - Faithfulness: {scores.get('faithfulness', 0.0):.3f}, "
                        f"Template: {scores.get('template', 0.0):.3f}, "
                        f"Medical Facts: {scores.get('medical_facts', 0.0):.3f}, "
                        f"ROUGE: {scores.get('rouge', 0.0):.3f}, "
                        f"Hallucination: {scores.get('hallucination', 0.0):.3f}, "
                        f"G-Eval: {scores.get('geval', 0.0):.3f}")
                
                # Log critical issues if any
                issues = validation_result.get("issues", {})
                critical_issues_count = sum(len(issue_list) for issue_list in issues.values())
                if critical_issues_count > 0:
                    add_log(job_id, "clinical_validation", "warning",
                            f"Validation found {critical_issues_count} issue(s) across all categories")
            else:
                add_log(job_id, "clinical_validation", "warning",
                        "Skipping validation - no generated output available")
        
        except Exception as validation_error:
            # Don't fail processing if validation fails - just log the error
            add_log(job_id, "clinical_validation", "warning",
                    f"Validation failed but continuing processing: {str(validation_error)}")
            validation_result = None
