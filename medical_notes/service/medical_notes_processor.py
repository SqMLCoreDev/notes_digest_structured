...
def extract_structured_data(rawdata, note_type):
    """
    ...
    try:
        ...
        # Process using the existing generator
        result, error = notes_generator.process_medical_records(df, note_type=note_type)
...
def prepare_es_record(note_data, note_type, processed_text=None, processed_json=None, 
    ...
    if not demographics.get('patient_name') and processed_text:
        print(f"    ⚠️ Patient name not found in notes_digest, trying processed_text fallback")
        fallback_demographics, fallback_issues = extract_demographics_from_text(processed_text)
    ...
    # Fallback 3: Use LLM to extract patient name from raw data if not found
    if not demographics.get('patient_name'):
        rawdata = note_data.get('rawdata', '')
        if rawdata:
            print(f"    ⚠️ Patient name not found in notes_digest or processed_text, trying LLM extraction from rawdata")
            from medical_notes.service.note_type_extractor import extract_patient_name
            llm_patient_name = extract_patient_name(rawdata)
            if llm_patient_name:
    ...
    # Fallback 4: Use patientID from clinical notes if patient name still not found
    if not demographics.get('patient_name'):
        clinical_patient_id = note_data.get('patientID', '')
        if clinical_patient_id:
            demographics['patient_name'] = clinical_patient_id
            print(f"    ✓ Using patientID from clinical notes as final fallback: '{clinical_patient_id}'")
            # Remove the patient name issue since we found it in clinical notes
            demo_issues = [issue for issue in demo_issues if 'Patient name not found' not in issue]
        else:
            print(f"    ⚠️ PatientID not available in clinical notes for patient name fallback")
...
def execute_parallel_processing(packaged_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        processed_text, soap_text, notes_digest, error = extract_structured_data(raw_data, note_type)
